...
private BooksSqlOpenHelp sqlOpenHelp;
	...
	public void setBookPageFactory(BookPageFactory bookPageFactory) {
		mBookPageFactory = bookPageFactory;
		...
		restartFromMark();
		mBookPageFactory.Draw(mCurPageCanvas);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (!menushowing) {// menushowing

				if (inShowMeneArea(x0, y0)) {// 点击显示菜单区
				...
				if (DragToRight()) {
					...
					if (mBookPageFactory.isfirstPage())
						return false;
					mBookPageFactory.Draw(mNextPageCanvas);
				} else {
					try {
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) {
			...
			postInvalidate();
			// if (isAnimationFinished()&&startAnimationflag) {
			// mBookPageFactory.Draw(mCurPageCanvas);
	...
	public void sava() {
//		getContext().getSharedPreferences("bookmark", Context.MODE_PRIVATE).edit()
//				.putInt(mBookPageFactory.getBookName(), mBookPageFactory.getP()).commit();
		
		sqlOpenHelp.update(mBookPageFactory.getBookPath(), mBookPageFactory.getP());
		
		

	}

	public void restartFromMark() {
//		if (p < 0) {
//			mBookPageFactory.setP(getContext().getSharedPreferences("bookmark", Context.MODE_PRIVATE)
//					.getInt(mBookPageFactory.getBookName(), 0));
//		} else {
//			mBookPageFactory.setP(p);
//		}
		int mark=sqlOpenHelp.getMark(mBookPageFactory.getBookPath());
		
		mBookPageFactory.setP(mark);
	}
